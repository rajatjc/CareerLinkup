...
from CareerLinkup.forms import Applicant_Add, Applicant_Update, Employer_Add, Employer_Update, User_Login, Admin_Update, Admins_Add, Admins_Edit, Job_Add, Job_Update, Forgot_Password, Reset_Password
from CareerLinkup.models import Admin, User, Job, Employer, Applicant, jobs_applied
from CareerLinkup.utils import user_role_required, save_file, remove_file, send_pwd_reset_email
...
def admin_jobs_applied():
    ...
    return render_template("admin/jobs_applied.html", title="CareerLinkup | Applied Jobs", jobs=jobs, user=user)
...
def reports():
    ...
    title1 = 'Summery Report on CareerLinkup'
...
def admin_login():
    ...
    return render_template("admin/login.html", title="CareerLinkup | Admin Login", form=form)
...
def admin_create():
    # Check if there are any admins in the database. If yes, redirect to the admin login page.
    if Admin.query.count() > 0:
        flash("Initial admin user already exists. Please use the admin login page.", 'info')
        return redirect(url_for('.admin_login'))

    # If there are no admins in the database, create the initial admin user.
    form = Admins_Add()
    if form.validate_on_submit():
        # Hash the password
        pw_hash = bcrypt.generate_password_hash(form.password.data).decode('utf-8')

        # Create a new user with the provided email, password, and user role as 'admin'
        user = User(email=form.email.data.lower(), user_role='admin', password=pw_hash)
        db.session.add(user)

        # Create a new admin with the provided name, phone, and link it to the user
        admin_user = Admin(name=form.name.data.strip().capitalize(), phone=form.phone.data, user=user)
        ...
        flash('Initial Admin Account Created Successfully!', 'success')
        return redirect(url_for('.admin_account'))

    return render_template("admin/create.html", title="CareerLinkup | Create Initial Admin", form=form)