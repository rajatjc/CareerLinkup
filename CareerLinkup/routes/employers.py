...
from CareerLinkup.models import User, Employer, Job, jobs_applied, Applicant
from CareerLinkup.utils import save_file, user_role_required, remove_file, send_pwd_reset_email
from flask_login import login_user, current_user

employer = Blueprint('employer', __name__)


# Employer/company details/profile
@employer.route("/company/<int:company_id>/profile/")
def profile(company_id):
    company = Employer.query.get_or_404(company_id)
    return render_template("employers/profile.html", title="CareerLinkup | Company Profile", company=company)
...
def company_list():
    ...
    return render_template("employers/list.html", title="CareerLinkup | Companies", companies=companies, head=head, form1=form1)
...
def job_details(job_id):
    ...
    return render_template("employers/job-details.html", title="CareerLinkup | Job Details", job=job, applicants=applicants)
...
def settings():
    ...
    return render_template("employers/settings.html", title="CareerLinkup | Employer Settings", form=form, user = user)
...
def employer_login():
    ...
    return render_template("employers/login.html", title="CareerLinkup | Employer Login", form=form)
...
def employer_signup():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('.employer_account'))        
    return render_template("employers/signup.html", title="CareerLinkup | Employer Signup", form=form)
...
def company_search():
    ...
    return render_template("employers/list.html", title="CareerLinkup | Company Search", companies=companies, head=head, form1=form1)
...
def posted_jobs():
    ...
    return render_template("employers/jobs.html", title="CareerLinkup | Posted Jobs", jobs=jobs, list=list)
...
def applicants_per_job(job_id):
    ...
    return render_template("employers/filtered.html", title="CareerLinkup | Applicants per Job", applicants=applicants, job=job)
...
def list_applicant(job_id, applicant_id):
    ...
    try:
        # send_shortlist    (email, dict(list))
        db.session.commit()
